<?php
/**
 * Script para inserir tipos de veículos de exemplo (HOMOLOGAÇÃO)
 * Execute via navegador: https://floripa.in9automacao.com.br/hml_inserir_tipos_veiculo.php
 */
...
try {
    ...
    foreach ($tipos as $tipo) {
        ...
        $stmt = $pdo->prepare("SELECT id FROM bbb_tipos_veiculo WHERE id = :id OR nome = :nome LIMIT 1");
        $stmt->execute(['id' => $tipo['id'], 'nome' => $tipo['nome']]);
        $existe = $stmt->fetch();
        
        if (!$existe) {
            $sql = "INSERT INTO `bbb_tipos_veiculo` (`id`, `nome`, `descricao`, `ativo`, `icone`, `usuario_id`) 
